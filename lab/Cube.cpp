...
HRESULT Cube::Init(ID3D11Device* device, ID3D11DeviceContext* context, int screenWidth, int screenHeight) {
    ...
    bd.ByteWidth = sizeof(CubeVertex) * vertices.size();
...
HRESULT Cube::Update(ID3D11DeviceContext* context, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix, XMMATRIX& projectionMatrix, XMVECTOR& cameraPos, std::vector<Light>& lights) {
    WorldMatrixBuffer worldMatrixBuffer;
    worldMatrixBuffer.worldMatrix = worldMatrix;
    worldMatrixBuffer.color = XMFLOAT4(cubeMaterial.shine, 0.0f, 0.0f, 0.0f);
